...
export function UploadButton({ spaceId, onUploaded, text = "Upload", colorScheme = "green", assetId, width, height, type, positiveImageButtonText = "ADD IMAGE ASSET" }: { text?: string, spaceId: string, colorScheme?: "green" | "blue" | "gray", onUploaded: (asset: Asset) => void, assetId?: string, width?: number, height?: number, type?: "file" | "image", positiveImageButtonText?: string }) {
    ...
    return <>
        ...
        >
            ...
            <ModalContent w="800px" minW="880px">
                ...
                <ModalFooter pb={10} px={10} gap={10}>
                    ...
                    }}>{positiveImageButtonText}</Button>
                    <Button
                        variant="ghost"
                        onClick={() => {
                            setImageEditorOpen(false);
        ...
        <Button colorScheme={colorScheme} width="150px" isLoading={addingLoading} isDisabled={addingLoading} onClick={() => {
            inputRef.current?.click();

        }}>{text}</Button>
        <input type="file" onChange={handleInputChange} accept={type === "image" ? "image/png, image/gif, image/jpeg" : undefined} ref={inputRef} style={{ display: "none" }}></input>
