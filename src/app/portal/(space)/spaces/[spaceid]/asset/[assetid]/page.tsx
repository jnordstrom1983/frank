...
import { Box, Button, Center, Container, Flex, Image, Grid, GridItem, HStack, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Spinner, Table, Tag, Tbody, Td, Th, Thead, Tr, VStack, useDisclosure, useToast } from "@chakra-ui/react"
...
export default function Home({ params }: { params: { spaceid: string; assetid: string } }) {
                        ...
                        <Container maxW="1000px">
                            ...
                            <VStack w="100%" spacing="60px">
                                ...
                                <Grid templateColumns="3fr 2fr" rowGap="60px" columnGap={20} w="100%">
                                    <GridItem>
                                        <VStack w="100%" alignItems="flex-start" spacing={"60px"}>
                                            <TextInput subject="Description" type="textarea" placeholder="My asset description " value={description} onChange={setDescription}></TextInput>
                                            <TextInput subject="Folder" type="select" value={folder} onChange={setFolder} options={[{ key: "", text: "No folder" }, ...(folders || []).map(f => ({ key: f.folderId, text: f.name }))]}></TextInput>
                                            <SimpleCheckboxInput checked={enabled} onChange={setEnabled} subject="Enabled" description="Asset can be assigned to new content"></SimpleCheckboxInput>
                                        ...
                                        <VStack w="100%" alignItems="flex-start" spacing={"60px"}>
                                            ...
                                            <Box w="100%">
                                                <VStack w="100%" alignItems="flex-start">
                                                    ...
                                                    {asset.type === "image" && <Box>
                                                        <Box>Image</Box>
                                                        <Box borderRadius={"3px"} overflow="hidden">
                                                            <Image src={url} w="100%"></Image>
                                                        </Box>
                                                    </Box>}
                                                    {asset.type === "image" && <HStack>


                                                        <UploadButton positiveImageButtonText="Replace image" type={asset.type === "image" ? "image" : "file"} assetId={asset.assetId} colorScheme="blue" text={`REPLACE ${asset.type === "image" ? "IMAGE" : "FILE"}`} spaceId={params.spaceid} onUploaded={(asset) => {
                                                            setFilename(asset.filename);
                                                            setUrl(asset.url);
                                ...
                                <Box w="100%">
                                    ...
                                    <Button leftIcon={<Trash></Trash>} onClick={onDeleteOpen}>Delete asset</Button>
                                </Box>
    ...
    function EditButton({ }) {
        ...
        return <>
                ...
                <ModalContent w="800px" minW="880px">
                    ...
                    <ModalBody p={10}>
                        <Box w="800px" minW="800px">
                            <ImageEditor url={imagePreviewUrl} onDataChanged={({ rotation, mirrorX, mirrorY, cropX, cropY, cropHeight, cropWidth }) => {
                    ...
                    <ModalFooter pb={10} px={10} gap={10}>
                        <Button colorScheme="blue" isLoading={addingLoading} isDisabled={addingLoading} onClick={async () => {
            ...
            <Button colorScheme="blue" width="150px" isLoading={addingLoading} isDisabled={addingLoading} onClick={() => {
